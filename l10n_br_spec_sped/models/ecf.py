...
Ver pagina 49""",
    ...
    sit_especial = fields.Char(
        ...
        required=True,
        help=r"""Indicador de Situação Especial e Outros Eventos:  0 – Normal (Sem ocorrência de situação especial ou evento)  1 – Extinção 2 – Fusão 3 – Incorporação \ Incorporada 4 – Incorporação \ Incorporadora 5 – Cisão Total 6 – Cisão Parcial 8 – Desenquadramento de Imune/Isenta;  9 – Inclusão no Simples Nacional
Ver pagina 49""",
    )
    pat_reman_cis = fields.Monetary(
        "Patrimônio Remanescente em Caso de Cisão (%)", digits=4
    )
...
Ver pagina 50""",
    ...
    num_rec = fields.Char(
        "Número do Recibo da ECF Anterior (hashcode)",
...
class Registro0020(models.Model):
    ...
    ind_aliq_csll = fields.Integer(
        "PJ Sujeita à Alíquota da CSLL de 9% ou 17% ou 20% em 31/12/2015", required=True
    )
    ind_qte_scp = fields.Integer(
        "Quantidade total de SCP do Sócio Ostensivo de SCP", required=True
    )
...
Ver pagina 91""",
    ...
    reg_c050_ids = fields.One2many(
        "l10n.br.sped.ecf.c050",
        "parent_c040_id",
        string="Plano de Contas da ECD",
        help="Bloco C",
    )
    reg_c100_ids = fields.One2many(
        "l10n.br.sped.ecf.c100",
        "parent_c040_id",
        string="Centro de Custos",
        help="Bloco C",
    )
    reg_c150_ids = fields.One2many(
        "l10n.br.sped.ecf.c150",
        "parent_c040_id",
        string="Identificação  do  Período  dos  Saldos  Periódicos  das Contas Patrimoniais",
        help="Bloco C",
    )
    reg_c350_ids = fields.One2many(
        "l10n.br.sped.ecf.c350",
        "parent_c040_id",
...
Ver pagina 92""",
    ...
    parent_c040_id = fields.Many2one(
        "l10n.br.sped.ecf.c040", string="Identificador da ECD"
    )
    reg_c051_ids = fields.One2many(
        "l10n.br.sped.ecf.c051",
        "parent_c050_id",
        string="Plano de Contas Referencial Preenchido na ECD",
        help="Bloco C",
    )
    reg_c053_ids = fields.One2many(
        "l10n.br.sped.ecf.c053",
        "parent_c050_id",
...
Ver pagina 93""",
    ...
    parent_c050_id = fields.Many2one(
        "l10n.br.sped.ecf.c050", string="Plano de Contas da ECD"
    )
...
Ver pagina 94""",
    ...
    cod_cnt_corr = fields.Char(
        "Código da Subconta Correlata  (Deve estar no plano de contas)",
        required=True,
...
class RegistroC150(models.Model):
    ...
    reg_c155_ids = fields.One2many(
        "l10n.br.sped.ecf.c155",
        "parent_c150_id",
...
Ver pagina 97""",
    ...
    vl_deb = fields.Monetary(
        "Valor Total dos Débitos no Período.", required=True, digits=2
    )
    vl_cred = fields.Monetary(
        "Valor Total dos Créditos no Período.", required=True, digits=2
    )
...
Ver pagina 97""",
    ...
    parent_c150_id = fields.Many2one(
        "l10n.br.sped.ecf.c150",
        string="Identificação  do  Período  dos  Saldos  Periódicos  das Contas Patrimoniais",
    )
    reg_c157_ids = fields.One2many(
        "l10n.br.sped.ecf.c157",
        "parent_c155_id",
...
Ver pagina 98""",
    ...
    vl_sld_fin = fields.Monetary(
        "Valor do Saldo Final do Período Transferido", required=True, digits=2
    )
    ind_vl_sld_fin = fields.Char(
...
Ver pagina 98""",
    ...
    parent_c155_id = fields.Many2one(
        "l10n.br.sped.ecf.c155",
        string="Detalhes dos Saldos Contábeis das Contas Patrimoniais",
    )
...
class RegistroC350(models.Model):
    ...
    reg_c355_ids = fields.One2many(
        "l10n.br.sped.ecf.c355",
        "parent_c350_id",
...
class RegistroC355(models.Model):
    ...
    _description = (
        u"""Detalhes dos Saldos das Contas de Resultado Antes do Encerramento"""
    )
    _inherit = "l10n.br.sped.mixin"
    cod_cta = fields.Char(
...
Ver pagina 103""",
    ...
    desc_cta_ref = fields.Char(
        "Descrição da Conta Referencial.",
        required=True,
        help="""Descrição da Conta Referencial.
Ver pagina 103""",
    )
    val_cta_ref = fields.Monetary(
        "Valor Total da Conta Referencial.", required=True, digits=2
    )
...
Ver pagina 104""",
    ...
    parent_e010_id = fields.Many2one(
        "l10n.br.sped.ecf.e010", string="Saldos Finais Recuperados da ECF Anterior"
    )
...
Ver pagina 109""",
    ...
    parent_e030_id = fields.Many2one(
        "l10n.br.sped.ecf.e030", string="Identificação do Período"
    )
...
Ver pagina 116""",
    ...
    cod_cta_ref = fields.Char(
        "Código da Conta no Plano de Contas Referencial Definido em 0010",
        required=True,
        help="""Código da Conta no Plano de Contas Referencial Definido em 0010.COD_QUALIF_PJ, conforme tabela publicada no Sped.
Ver pagina 116""",
    )
    parent_j050_id = fields.Many2one(
        "l10n.br.sped.ecf.j050", string="Plano de Contas do Contribuinte"
    )
...
Ver pagina 131""",
    ...
    vl_sld_fin = fields.Monetary(
        "Valor do Saldo Final Mapeado.", required=True, digits=2
    )
    ind_vl_sld_fin = fields.Char(
        "Indicador da Situação do Saldo Final",
        required=True,
...
Ver pagina 131""",
    ...
    parent_k155_id = fields.Many2one(
        "l10n.br.sped.ecf.k155",
        string="Detalhes dos Saldos Contábeis (Depois do Encerramento do Resultado do Período)",
    )
...
Ver pagina 135""",
    ...
    parent_k355_id = fields.Many2one(
        "l10n.br.sped.ecf.k355",
        string="Saldos  Finais  das  Contas  Contábeis  de  Resultado Antes do Encerramento",
    )
...
Ver pagina 199""",
    ...
    val_cta_ref_ini = fields.Monetary(
        "Saldo Inicial da Conta Referencial.", required=True, digits=2
    )
...
Ver pagina 200""",
    ...
    parent_l030_id = fields.Many2one(
        "l10n.br.sped.ecf.l030",
        string="Identificação dos Períodos e Formas de Apuração do IRPJ e da CSLL no Ano-Calendário",
    )
...
Ver pagina 204""",
    ...
    descricao = fields.Char(
        "Descrição da Conta de Custos.",
        help="""Descrição da Conta de Custos.
Ver pagina 204""",
    )
    valor = fields.Monetary(
        "Saldo Final da Conta de Custos Antes do Encerramento", digits=2
    )
    parent_l030_id = fields.Many2one(
        "l10n.br.sped.ecf.l030",
        string="Identificação dos Períodos e Formas de Apuração do IRPJ e da CSLL no Ano-Calendário",
...
Ver pagina 206""",
    ...
    valor = fields.Monetary(
        "Saldo Final da Conta Referencial.", required=True, digits=2
    )
...
Ver pagina 218""",
    ...
    parent_m030_id = fields.Many2one(
        "l10n.br.sped.ecf.m030",
...
class RegistroM305(models.Model):
    ...
    _description = (
        u"""Contas  da  Parte  B  Relacionadas  ao  Lançamento  da Parte A do e-Lalur"""
    )
    _inherit = "l10n.br.sped.mixin"
    cod_cta_b = fields.Char(
        "Código da Conta na Parte B",
        required=True,
...
Ver pagina 223""",
    ...
    ind_vl_cta = fields.Char(
        "Indicador do Valor Total dos Lançamentos",
        required=True,
...
Ver pagina 223""",
    ...
    parent_m300_id = fields.Many2one(
        "l10n.br.sped.ecf.m300", string="Demonstração do Lucro Real"
    )
...
class RegistroM310(models.Model):
    ...
    _description = (
        u"""Contas  Contábeis  Relacionadas  ao  Lançamento  da Parte A do e-Lalur"""
    )
    _inherit = "l10n.br.sped.mixin"
    cod_cta = fields.Char(
...
Ver pagina 225""",
    ...
    cod_ccus = fields.Char(
        "Código do Centro de Custos (deve existir no J100)",
        help="""Código do Centro de Custos (deve existir no J100).
Ver pagina 225""",
    )
    vl_cta = fields.Monetary(
        "Valor da Conta Utilizado no Lançamento da Parte A", required=True, digits=2
    )
    ind_vl_cta = fields.Char(
...
Ver pagina 227""",
    ...
    parent_m310_id = fields.Many2one(
        "l10n.br.sped.ecf.m310",
        string="Contas  Contábeis  Relacionadas  ao  Lançamento  da Parte A do e-Lalur",
    )
...
class RegistroM355(models.Model):
    ...
    _description = (
        u"""Contas da Parte B Relacionadas ao ao Lançamento da Parte A do e-Lacs"""
    )
    _inherit = "l10n.br.sped.mixin"
    cod_cta_b = fields.Char(
        "Código da Conta na Parte B",
...
Ver pagina 235""",
    ...
    parent_m350_id = fields.Many2one(
        "l10n.br.sped.ecf.m350", string="Demonstração da Base de Cálculo da CSLL"
    )
...
class RegistroM360(models.Model):
    ...
    _description = (
        u"""Contas  Contábeis  Relacionadas  ao  Lançamento  da Parte A do e-Lacs"""
    )
    _inherit = "l10n.br.sped.mixin"
    cod_cta = fields.Char(
        "Código da Conta Contábil (Plano de Contas da Pessoa Jurídica)",
...
Ver pagina 239""",
    ...
    parent_m360_id = fields.Many2one(
        "l10n.br.sped.ecf.m360",
        string="Contas  Contábeis  Relacionadas  ao  Lançamento  da Parte A do e-Lacs",
    )
...
Ver pagina 243""",
    ...
    parent_m410_id = fields.Many2one(
        "l10n.br.sped.ecf.m410",
        string="Lançamentos na Conta da Parte B do e-Lalur e do e-Lacs Sem Reflexo na Parte A",
    )
...
class RegistroM500(models.Model):
    ...
    _description = (
        u"""Controle de Saldos das Contas da Parte B do e-Lalur e do e-Lacs"""
    )
    _inherit = "l10n.br.sped.mixin"
    cod_cta_b = fields.Char(
        "COD_CTA_B",
...
Ver pagina 244""",
    ...
    sd_ini_lal = fields.Monetary(
        "Saldo Inicial da Conta no Período de Apuração", required=True, digits=2
    )
...
Ver pagina 244""",
    ...
    ind_vl_lcto_parte_b = fields.Char(
        "IND_VL_LCTO_PARTE_B",
        required=True,
...
Ver pagina 245""",
    ...
    sd_fim_lal = fields.Monetary(
        "Saldo Final da Conta no Período de Apuração", required=True, digits=2
    )
...
Ver pagina 251""",
    ...
    parent_n030_id = fields.Many2one(
        "l10n.br.sped.ecf.n030",
        string="Identificação do Período e Forma de Apuração do IRPJ e da CSLL das Empresas Tributadas pelo Lucro Real",
    )
...
class RegistroN615(models.Model):
    ...
    per_incen_finor = fields.Monetary(
        "Percentual do Incentivo FINOR (até 6%).", required=True, digits=4
    )
    vl_liq_incen_finor = fields.Monetary(
        "Valor Líquido do Incentivo FINOR.", required=True, digits=2
    )
    per_incen_finam = fields.Monetary(
        "Percentual do Incentivo FINAM (até 6%).", required=True, digits=4
    )
    vl_liq_incen_finam = fields.Monetary(
        "Valor Líquido do Incentivo FINAM.", required=True, digits=2
    )
...
Ver pagina 296""",
    ...
    parent_p030_id = fields.Many2one(
        "l10n.br.sped.ecf.p030",
...
class RegistroP400(models.Model):
    ...
    _description = (
        u"""Apuração da Base de Cálculo da CSLL com Base no Lucro Presumido"""
    )
    _inherit = "l10n.br.sped.mixin"
    codigo = fields.Char(
        "CODIGO",
...
class RegistroT120(models.Model):
    ...
    _description = (
        u"""Apuração  da  Base  de  Cálculo  do  IRPJ  com  Base  no Lucro Arbitrado"""
    )
    _inherit = "l10n.br.sped.mixin"
    codigo = fields.Char(
        "CODIGO",
...
Ver pagina 332""",
    ...
    valor = fields.Monetary("Valor", digits=2)
    parent_t030_id = fields.Many2one(
        "l10n.br.sped.ecf.t030",
...
class RegistroT150(models.Model):
    ...
    _description = (
        u"""Cálculo  do  Imposto  de  Renda  com  Base  no  Lucro Arbitrado"""
    )
    _inherit = "l10n.br.sped.mixin"
    codigo = fields.Char(
        "CODIGO",
...
class RegistroT170(models.Model):
    ...
    _description = (
        u"""Apuração da Base de Cálculo da CSLL com Base no Lucro Arbitrado"""
    )
    _inherit = "l10n.br.sped.mixin"
    codigo = fields.Char(
        "CODIGO",
...
Ver pagina 350""",
    ...
    parent_u030_id = fields.Many2one(
        "l10n.br.sped.ecf.u030",
...
Ver pagina 398""",
    ...
    nome = fields.Char(
        "Nome/Razão Social da Entidade Integrante",
        required=True,
...
Ver pagina 399""",
    ...
    num_tel = fields.Char(
        "Número de telefone completo da entidade integrante reportada",
...
Ver pagina 400""",
    ...
    ativ_4 = fields.Char(
        "Manufatura ou Produção: S - Sim N - Não",
        required=True,
...
Ver pagina 400""",
    ...
    ativ_6 = fields.Char(
        "Serviços Administrativos, de Gestão ou de Suporte",
        required=True,
        help="""Serviços Administrativos, de Gestão ou de Suporte: S - Sim N - Não
Ver pagina 400""",
    )
    ativ_7 = fields.Char(
        "Prestação de Serviços a Partes Não Relacionadas",
        required=True,
...
Ver pagina 400""",
    ...
    ativ_11 = fields.Char(
        "Gestão de Ações e Outros Instrumentos de Capital",
        required=True,
...
Ver pagina 401""",
    ...
    desc_outros = fields.Char(
        "Descrição da Atividade Econômica Desempenhada, no caso de W250",
...
Ver pagina 401""",
    ...
    parent_u182_id = fields.Many2one(
        "l10n.br.sped.ecf.u182", string="Cálculo da CSLL das Empresas Imunes ou Isentas"
    )
...
Ver pagina 404""",
    ...
    ind_rec_nao_rel = fields.Char(
        "O campo 'Receitas Provenientes de Partes Não Relacionadas' (W200",
...
Ver pagina 404""",
    ...
    ind_rec_rel = fields.Char(
        "O campo 'Receitas Provenientes de Partes Relacionadas' (W200",
...
Ver pagina 405""",
    ...
    ind_luc_prej_antes_ir = fields.Char(
        "O campo 'Lucro ou Prejuízo antes do IR' (W200",
...
Ver pagina 405""",
    ...
    ind_ir_pago = fields.Char(
        "O campo 'Imposto de Renda Pago' (W200",
...
Ver pagina 405""",
    ...
    ind_ir_devido = fields.Char(
        "O campo 'Imposto de Renda Devido' (W200",
...
Ver pagina 405""",
    ...
    ind_num_emp = fields.Char(
        "O campo 'Número de Empregados' (W200",
...
Ver pagina 405""",
    ...
    fim_observacao = fields.Char(
        "Indicador de fim das observações",
...
Ver pagina 416""",
    ...
    ato_conc = fields.Char(
        "Ato Concessório - Natureza e Número",
        required=True,
...
Ver pagina 435""",
    ...
    parent_x300_id = fields.Many2one(
        "l10n.br.sped.ecf.x300",
...
class RegistroX320(models.Model):
    ...
    _description = (
        u"""Operações  com  o  Exterior  -  Importações  (Saídas  de Divisas)"""
    )
    _inherit = "l10n.br.sped.mixin"
    num_ord = fields.Char(
...
Ver pagina 448""",
    ...
    vl_oper = fields.Monetary("Valor da Operação", required=True, digits=2)
    cond_pes = fields.Integer("Condição da Pessoa Envolvida na Operação", required=True)
    parent_x320_id = fields.Many2one(
        "l10n.br.sped.ecf.x320",
...
Ver pagina 451""",
    ...
    ind_isen_petr = fields.Char(
        "Indicador de Prospecção e Exploração de Petróleo e Gás",
        required=True,
...
class RegistroX350(models.Model):
    ...
    _description = (
        u"""Participações  no  Exterior  -  Resultado  do  Período  de Apuração"""
    )
    _inherit = "l10n.br.sped.mixin"
    ...
    rec_auferidas = fields.Monetary(
        "Receitas Financeiras Auferidas com a Vinculada no Brasil", digits=2
    )
    rec_outras_oper = fields.Monetary("Outras Receitas Operacionais", digits=2)
    desp_brasil = fields.Monetary(
        "Despesas Financeiras Pagas ou Creditadas à Vinculada no Brasil", digits=2
    )
    ...
    luc_liq_ant_ir = fields.Monetary(
        "Lucro Líquido Antes do Imposto de Renda", digits=2
    )
    imp_dev = fields.Monetary("IMP_DEV", digits=2)
    luc_liq = fields.Monetary("Lucro líquido do Período de Apuração", digits=2)
    parent_x340_id = fields.Many2one(
        "l10n.br.sped.ecf.x340", string="Identificação da Participação no Exterior"
    )
...
class RegistroX351(models.Model):
    ...
    res_inv_per_real = fields.Monetary(
        "Resultado (positivo ou negativo) da Própria Investida em Reais", digits=2
    )
    ...
    res_neg_acum = fields.Monetary(
        "Resultado Negativo Acumulado de Anos Anteriores", digits=2
    )
    res_pos_trib = fields.Monetary("Resultado Positivo a Tributar", digits=2)
    res_pos_trib_real = fields.Monetary(
        "Resultado Positivo a Tributar em Reais (R$)", digits=2
    )
    ...
    imp_ret_ext_real = fields.Monetary(
        "Imposto Retido na Fonte no Exterior em Reais", digits=2
    )
...
class RegistroX352(models.Model):
    ...
    res_per_real = fields.Monetary(
        "Resultado (positivo ou negativo) do Período em Reais", digits=2
    )
    luc_disp = fields.Monetary(
        "Lucro Disponibilizado no Período na Moeda do País de Domicílio", digits=2
    )
    luc_disp_real = fields.Monetary(
        "Lucro Disponibilizado no Período em Reais", digits=2
    )
    parent_x340_id = fields.Many2one(
        "l10n.br.sped.ecf.x340", string="Identificação da Participação no Exterior"
    )
...
class RegistroX353(models.Model):
    ...
    res_neg_util = fields.Monetary(
        "Resultado Negativo Utilizado na Consolidação na Moeda do País de", digits=2
    )
    res_neg_util_real = fields.Monetary(
        "Resultado Negativo Utilizado na Consolidação em Reais", digits=2
    )
    saldo_res_neg_nao_util = fields.Monetary(
        "Saldo do Resultado Negativo Não Utilizado na Moeda do País de", digits=2
    )
    parent_x340_id = fields.Many2one(
        "l10n.br.sped.ecf.x340", string="Identificação da Participação no Exterior"
    )
...
class RegistroX356(models.Model):
    ...
    _inherit = "l10n.br.sped.mixin"
    perc_part = fields.Monetary(
        "Percentual de Participação da Investidora Direta (%)", digits=4
    )
...
class RegistroX410(models.Model):
    ...
    ind_home_disp = fields.Char(
        "Indicador de Homepage Disponível",
        required=True,
...
Ver pagina 471""",
    ...
    ind_serv_disp = fields.Char(
        "Indicador de Disponibilidade de Servidor",
        required=True,
...
class RegistroX430(models.Model):
    ...
    vl_serv_sem_assist = fields.Monetary(
        "constituírem transferência de tecnologia, tais como", digits=2
    )
...
class RegistroX450(models.Model):
    ...
    vl_juro_pf = fields.Monetary(
        "Valor dos Juros sobre o Capital Próprio Pagos a Pessoa Física", digits=2
    )
    vl_juro_pj = fields.Monetary(
        "Valor dos Juros sobre o Capital Próprio Pagos a Pessoa Jurídica", digits=2
    )
    vl_demais_juros = fields.Monetary("Valor dos Demais Juros", digits=2)
...
class RegistroY520(models.Model):
    ...
    _inherit = "l10n.br.sped.mixin"
    tip_ext = fields.Char(
        "Tipo: R - Rendimentos Recebidos P – Pagamentos",
        required=True,
...
class RegistroY550(models.Model):
    ...
    _description = (
        u"""Vendas a Comercial Exportadora com Fim Específico de Exportação"""
    )
    _inherit = "l10n.br.sped.mixin"
    cnpj_exp = fields.Char(
...
Ver pagina 514""",
    ...
    cod_ncm = fields.Integer(
        "Código NCM (Nomenclatura Comum do Mercosul)", required=True
    )
    vl_venda = fields.Monetary(
        "Valor da Venda: Valor da mercadoria vendida", required=True, digits=2
    )
...
Ver pagina 515""",
    ...
    vl_compra = fields.Monetary(
        "Valor da Compra: Valor de aquisição do produto", digits=2
    )
...
class RegistroY600(models.Model):
    ...
    pais = fields.Integer(
        "País de Residência ou Domicílio da Pessoa Física ou Jurídica", required=True
    )
...
class RegistroY620(models.Model):
    ...
    _description = (
        u"""Participações Avaliadas pelo Método de Equivalência Patrimonial"""
    )
    _inherit = "l10n.br.sped.mixin"
    ...
    pais = fields.Integer(
        "País: País onde a coligada ou controlada está domiciliada", required=True
    )
    cnpj = fields.Char(
        "CNPJ: CNPJ da coligada ou controlada",
...
Ver pagina 537""",
    ...
    qte_quot = fields.Integer(
        "Quantidade de Quotistas no Final do Período", required=True
    )
    qte_quota = fields.Integer(
        "Quantidade de Quotas no Final do Período", required=True
    )
    patr_fin_per = fields.Monetary(
        "Patrimônio no Final do Período", required=True, digits=2
    )
...
Ver pagina 539""",
    ...
    cnpj_lid = fields.Char(
        "CNPJ da Empresa Líder do Consórcio.",
        required=True,
        help="""CNPJ da Empresa Líder do Consórcio.
Ver pagina 539""",
    )
    vl_decl = fields.Monetary(
        "Receita do Declarante no Consórcio", required=True, digits=2
    )
...
Ver pagina 542""",
    ...
    perc_pat_liq = fields.Monetary(
        "Percentual do Patrimônio Líquido Destinado", digits=4
    )
...
class RegistroY671(models.Model):
    ...
    vl_doa_crianca = fields.Monetary(
        "Doação aos Fundos dos Direitos da Criança e do Adolescente", digits=2
    )
    vl_doa_idoso = fields.Monetary(
        "Doação aos Fundos Nacional, Estaduais ou Municipais do Idoso", digits=2
    )
    ...
    vl_inc_ini = fields.Monetary(
        "Bens Sujeitos ao Incentivo de que Trata a Lei no 11", digits=2
    )
    vl_inc_fin = fields.Monetary(
        "Bens Sujeitos ao Incentivo de que Trata a Lei no 11", digits=2
    )
    vl_csll_deprec_ini = fields.Monetary(
        "Saldo de Créditos de CSLL sobre Depreciação no Início do Período", digits=2
    )
    vl_oc_sem_iof = fields.Monetary(
        "Valor das Operações de Câmbio com Isenção de IOF", digits=2
    )
    vl_folha_aliq_red = fields.Monetary("VL_FOLHA_ALIQ_RED", digits=2)
    vl_aliq_red = fields.Monetary(
        "Alíquota Reduzida de que Trata a Lei no 11", digits=4
    )
...
class RegistroY672(models.Model):
    ...
    vl_estoques = fields.Monetary(
        "Estoques: Valor dos estoques no fim do “Ano da Escrituração”", digits=2
    )
    ...
    vl_aplic_fin_ant = fields.Monetary(
        "Saldo de Aplicações Financeiras do Ano Anterior", digits=2
    )
    ...
    vl_compra_merc = fields.Monetary(
        "Compras de Mercadorias no Ano-Calendário", digits=2
    )
...
Ver pagina 554""",
    ...
    valor = fields.Monetary("Valor", digits=2)
    parent_y680_id = fields.Many2one(
        "l10n.br.sped.ecf.y680",
...
Ver pagina 556""",
    ...
    acres_patr = fields.Monetary(
        "Acréscimo Patrimonial no Mês.", required=True, digits=2
    )
...
class RegistroY720(models.Model):
    ...
    rec_brut_ant = fields.Monetary(
        "Receita bruta do período anterior.", required=True, digits=2
    )
...
Ver pagina 563""",
    ...
    hash = fields.Char(
        "Hash do arquivo incluído na escrituração",
        required=True,
...
Ver pagina 566""",
    ...
    conteudo = fields.Monetary(
        "Conteúdo: Conteúdo do campo – Valor preenchido pelo usuário", digits=2
    )
...
class Registro9900(models.Model):
    ...
    _inherit = "l10n.br.sped.mixin"
    reg_blc = fields.Char(
        "Registro: Registro que será totalizado em 9900",
        required=True,
