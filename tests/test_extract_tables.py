...
from spedextractor import download as extract_download_module # For mocking download calls from extract_tables
...
def mock_specs_extract(tmp_path, monkeypatch):
    ...
    if hasattr(extract_download_module, "SPECS_PATH"): # Check if download module used by extract_tables has it
...
def mock_download_pdf_in_extract(mocker) -> mock.Mock:
    ...
    return mocker.patch("spedextractor.extract_tables.download.download_mod_pdf", return_value=True)
...
def test_extract_mod_tables_pdf_needs_download(
    mock_specs_extract, mock_download_pdf_in_extract, mock_camelot_read_pdf, dummy_pdf_file, caplog
):
    mod_name = "ecf_new"
    year = 2023
...
def TODOtest_extract_cli_specific_module(mocker, mock_specs_extract, dummy_pdf_file, mock_camelot_read_pdf, caplog): # Add fixture as param
    ...
    mock_dl = mocker.patch("spedextractor.extract_tables.download.download_mod_pdf", return_value=True)
    ...
    expected_pdf_path_str = str((mock_specs_extract / "2023" / "pdf" / "ecd.pdf").resolve())
    assert expected_pdf_path_str == args[0]
    assert kwargs.get("pages") == "1-1"
    mock_dl.assert_not_called()

def TODOtest_extract_cli_all_modules(mocker, mock_specs_extract, dummy_pdf_file, mock_camelot_read_pdf, caplog): # Add fixture as param
    actual_mock_read_pdf_func, _ = mock_camelot_read_pdf # Unpack the mock for camelot.read_pdf

    mock_dl_pdf_in_extract_tables = mocker.patch("spedextractor.extract_tables.download.download_mod_pdf")
    ...
    def side_effect_download(mod_name_download, year_val_download):
        if mod_name_download == "ecf":
            ...
            with open(pdf_dir / f"{mod_name_download}.pdf", "wb") as f_out:
                writer.write(f_out)
            return True
        elif mod_name_download == "ecd" and (mock_specs_extract / str(year_val_download) / "pdf" / f"{mod_name_download}.pdf").exists():
             return True
        return False
    ...
    runner = CliRunner()
    result = runner.invoke(extract_tables.main_command, ["--year", "2023"], catch_exceptions=False)

    if result.exception and not isinstance(result.exception, SystemExit):
        raise result.exception
    assert result.exit_code == 0, f"CLI failed. Output: {result.output}\nException: {result.exception}\nLogs: {caplog.text}"

    assert actual_mock_read_pdf_func.call_count == len(test_cli_modules) # Use the unpacked mock

    calls_to_download = mock_dl_pdf_in_extract_tables.call_args_list
    assert mock.call("ecf", 2023) in calls_to_download
    assert not any(call_args[0] == "ecd" for call_args, _ in calls_to_download), \
