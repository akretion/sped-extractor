...
def test_is_register_code():
    ...
    assert build_csv._is_register_code("A12") is False # Correct, needs 2 digits in middle
...
def test_get_raw_rows(mock_specs_path_build_csv, mock_extract_mod_tables):
    ...
    create_dummy_raw_csv_file(raw_csv_dir, f"{mod}-page-10-table-1.csv", [["r1c1", "r1c2"], ["r2c1", "r2c2"]])
    create_dummy_raw_csv_file(raw_csv_dir, f"{mod}-page-5-table-1.csv", [["p5r1c1", "p5r1c2"]])
...
def test_get_raw_rows_calls_extract_if_missing(mock_specs_path_build_csv, mock_extract_mod_tables):
    mod = "ecd"
    year = 2023
    ...
    def simulate_extraction_creates_dir_and_file(m, y):
        ...
        create_dummy_raw_csv_file(dir_to_create, f"{m}-page-1-table-1.csv", [["dummy data"]])
...
def test_extract_registers_list_ecd(ecd_raw_rows_for_registers):
    mod = "ecd"
    year = 2023
    registers = build_csv.extract_registers_list(mod, year, raw_rows=ecd_raw_rows_for_registers)
...
def ecd_raw_rows_for_0000_fields_fixture(): # Renamed for clarity
    return {
        ...
        55: [
            ["10", "IM", "Inscrição Municipal...", "C", "-", "-", "-", "Não", "[REGRA_CAMPO_CARACTERE_INVALIDO]"],
        ],
        56: [
             ["Nº", "Campo", "Descrição", "Tipo", "Tamanho", "Decimal", "Valores Válidos", "Obrigatório", "Regras de Validação do Campo"],
             ["01", "REG", "Texto fixo contendo “0001”.", "C", "004", "-", "“0001”", "Sim", "-"], # Stop condition
        ]
    }

def test_extract_register_fields_ecd_0000(ecd_raw_rows_for_0000_fields_fixture, ecd_module_header_fixture):
    mod = "ecd"
    year = 2023
    register_name = "0000"
    with mock.patch("spedextractor.build_csv._get_mod_header", return_value=ecd_module_header_fixture):
        fields = build_csv.extract_register_fields(mod, year, register_name, raw_rows=ecd_raw_rows_for_0000_fields_fixture, patch=False)
...
def ecd_registers_for_get_registers():
    return [
        ...
        {"block": "0", "code": "0001", "desc": "ABERTURA BLOCO 0...", "level": 1, "card": "1"},
    ]
...
def test_get_registers_ecd(mock_ecd_accurate_fields_csv, ecd_module_header_fixture, ecd_registers_for_get_registers):
    mod = "ecd"
    year = 2023
    with mock.patch("spedextractor.build_csv._get_mod_header", return_value=ecd_module_header_fixture), \
         mock.patch("spedextractor.build_csv.get_raw_rows", return_value={}), \
         mock.patch("spedextractor.build_csv.extract_registers_list", return_value=list(ecd_registers_for_get_registers)): # ensure it's a list copy
        registers = build_csv.get_registers(mod, year, extracted_registers=list(ecd_registers_for_get_registers)) # pass a copy
    assert len(registers) == 2

def test_build_usable_fields_csv_ecd(mock_ecd_accurate_fields_csv, mock_specs_path_build_csv, ecd_module_header_fixture):
    mod = "ecd"
    year = 2023
    with mock.patch("spedextractor.build_csv._get_mod_header", return_value=ecd_module_header_fixture):
...
def test_build_registers_csv_ecd(mock_ecd_accurate_fields_csv, mock_specs_path_build_csv, ecd_module_header_fixture, ecd_registers_for_get_registers):
    mod = "ecd"
    year = 2023
    with mock.patch("spedextractor.build_csv._get_mod_header", return_value=ecd_module_header_fixture), \
         mock.patch("spedextractor.build_csv.get_raw_rows", return_value={}), \
         mock.patch("spedextractor.build_csv.extract_registers_list", return_value=list(ecd_registers_for_get_registers)):
        build_csv.build_registers_csv(mod, year, extracted_registers=list(ecd_registers_for_get_registers))
